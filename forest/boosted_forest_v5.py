...
'''
adjust alpha calculation so that it accomodates trees with error > .5  
use epsilon as a hyper parameter
'''
...
workdir = '/Users/ironchefnate/Library/Mobile Documents/com~apple~CloudDocs/Documents/USC/CSCI_699_HRI/project/code/robolog/boost/experiments/boosted_forest_v5'
with open(path.join(oracle_dir, oracle_file), 'rb') as f:
    Q = pickle.load(f)

params = dict(N=300, max_depth=5, target="CartPole-v2", forest_size=10, eps_thresh=1)
...
print(
    f'max depth: {params["max_depth"]}, number of samples: {params["N"]}, max trees {params["forest_size"]}, target: {params["target"]}')
F = StochasticArbor()
...
while num_trees > 0 and F.size() < params['forest_size']:
    ...
    eps = np.sum(weights * min_tree_loss_indicators) / np.sum(weights)
    print(f'eps: {eps}')
    alpha = .5 * np.log2((1 - .5 * eps) / (.5 * eps))
    if eps < params['eps_thresh']:
        T.beta = alpha
        ...
        #print(f'Loss: {oracle_rewards - min_tree_rewards}')
        ...
        print(f'mean reward: {np.mean(demo_rewards)}, max: {np.max(demo_rewards)}, min: {np.min(demo_rewards)}, var: {np.var(demo_rewards)}, std: {np.std(demo_rewards)}')
...
env.close()
# save F
filename = f'Forest_depth{params["max_depth"]}_samples{params["N"]}_cp2_00.pkl'
